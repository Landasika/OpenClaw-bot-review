...
function SessionList({ agentId }: { agentId: string }) {
  ...
  const [gateway, setGateway] = useState<GatewayInfo>({ port: 0 });
  const [error, setError] = useState<string | null>(null);
  const [loading, setLoading] = useState(true);
  ...
  useEffect(() => {
    fetch("/api/system-config")
      .then((r) => r.json())
      .then((cfg) => {
        if (typeof cfg.gatewayDefaultPort === "number") {
          setGateway((prev) => {
            if (prev.port > 0) return prev;
            return { ...prev, port: cfg.gatewayDefaultPort };
          });
        }
      })
      .catch(() => {});
