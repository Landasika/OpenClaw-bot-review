...
export default function PixelOfficePage() {
  ...
  const gatewayRef = useRef<{ port: number; token?: string; host?: string }>({ port: 0 })
  ...
  useEffect(() => {
    fetch("/api/system-config")
      .then((res) => res.json())
      .then((cfg) => {
        if (typeof cfg.gatewayDefaultPort === "number" && gatewayRef.current.port <= 0) {
          gatewayRef.current = { ...gatewayRef.current, port: cfg.gatewayDefaultPort }
        }
      })
      .catch(() => {})
  }, [])
  ...
  // Photograph is only shown after user clicks the camera furniture (not auto-loaded)
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        ...
        if (data.gateway && typeof data.gateway.port === "number") {
          gatewayRef.current = { port: data.gateway.port, token: data.gateway.token, host: data.gateway.host }
        }
