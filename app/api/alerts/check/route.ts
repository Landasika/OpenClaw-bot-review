...
function getAlertConfig(): AlertConfig {
  ...
  return {
    ...
    rules: systemConfig.alertsDefaultRules.map((rule) => ({ ...rule })),
    lastAlerts: {},
  };
}
...
function getGatewayConfig() {
  ...
  try {
    ...
    return { port: systemConfig.gatewayDefaultPort, token: "", feishu: {} };
  }
}
...
function getFeishuAccountForAgent(agentId: string, feishuConfig: any, bindings: any[]): { appId: string; appSecret: string; accountId: string } | null {
  ...
  if (agentId === systemConfig.defaultAgent && feishuConfig.enabled && feishuConfig.appId && feishuConfig.appSecret) {
    return { appId: feishuConfig.appId, appSecret: feishuConfig.appSecret, accountId: systemConfig.defaultAgent };
  }
...
async function sendAlert(agentId: string, message: string) {
  const systemConfig = getSystemConfig();
  const openclawConfig = getOpenclawConfig();
  const gatewayPort = openclawConfig.gateway?.port || systemConfig.gatewayDefaultPort;
