...
import { getAgentDisplayName, getAvailableAgents, getSystemConfig } from "@/lib/system-config";
...
function readIdentityName(
  ...
  agentDir?: string,
  workspace?: string,
  defaultAgentId: string = getSystemConfig().defaultAgent
): string | null {
  const candidates = [
    ...
    agentId === defaultAgentId ? path.join(OPENCLAW_DIR, `workspace/IDENTITY.md`) : null,
...
export async function GET() {
  ...
  try {
    ...
    if (agentList.length === 0) {
      ...
      // If still empty, fallback to system-config.json
      if (agentList.length === 0) {
        agentList = getAvailableAgents().map((id) => ({
          id,
    ...
    const agents = await Promise.all(agentList.map(async (agent: any) => {
      const id = agent.id;
      const identityName = readIdentityName(id, agent.agentDir, agent.workspace, primaryAgentId);
      const name = identityName || agent.name || getAgentDisplayName(id);
      ...
      if (id === primaryAgentId) {
        ...
        if (!hasFeishu && channels.feishu && channels.feishu.enabled !== false) {
          ...
          const acc = feishuAccounts[primaryAgentId];
          const appId = acc?.appId || channels.feishu?.appId;
          const userOpenId = feishuUserOpenIds[primaryAgentId] || null;
          platforms.push({ name: "feishu", accountId: primaryAgentId, appId, ...(userOpenId && { botOpenId: userOpenId }) });
        }
    ...
    const data = {
      ...
      gateway: {
        port: config.gateway?.port || systemConfig.gatewayDefaultPort,
