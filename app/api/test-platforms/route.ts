...
import { getAvailableAgents, getSystemConfig } from "@/lib/system-config";
...
export async function POST() {
  try {
    ...
    const primaryAgentId = systemConfig.defaultAgent;
    ...
    const gatewayPort = config.gateway?.port || systemConfig.gatewayDefaultPort;
    ...
    if (agentList.length === 0) {
      ...
      if (agentList.length === 0) {
        agentList = getAvailableAgents().map((id) => ({ id }));
      }
    }
    ...
    for (const agent of agentList) {
      ...
      if (account && account.appId && account.appSecret && !testedFeishuAccounts.has(accountId)) {
        ...
        if (id === primaryAgentId && feishuConfig.enabled && feishuConfig.appId && feishuConfig.appSecret && !testedFeishuAccounts.has(primaryAgentId)) {
          testedFeishuAccounts.add(primaryAgentId);
          const testUserId = getFeishuDmUser(primaryAgentId);
          platformTests.push(testFeishu(id, primaryAgentId, feishuConfig.appId, feishuConfig.appSecret, feishuDomain, testUserId));
        }
      }

      // Discord: only test once
      if (id === primaryAgentId && discordConfig.enabled && discordConfig.token) {
        platformTests.push(testDiscord(id, discordConfig.token, discordTestUser));
      }

      // Telegram: only test once
      if (id === primaryAgentId && telegramConfig.enabled && telegramConfig.botToken) {
      ...
      if (id === primaryAgentId && whatsappConfig && whatsappConfig.enabled !== false) {
